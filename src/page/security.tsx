...
const Security = () => {
  ...
  const logoutQuery = async () => logoutAxios();

  useEffect(() => {
    logoutQuery().then(() => null);
    clearToken();
  }, []);

  return (
    <div className="w-full h-screen flex flex-col items-center justify-center">
      <div className="m-4 p-8 flex flex-col items-center text-center">
        ...
        <div className="flex flex-col items-center justify-center">
          <p className="text-5xl md:text-6xl lg:text-4xl font-bold tracking-wider text-gray-600 mt-8">Sesión
            caducada</p>
          <p className="md:text-lg xl:text-xl text-gray-500 mt-4">Intente ingresar de nuevo</p>
          <Link to="/inicio"
                className="p-2 bg-gray-800 border bg-gray-800 rounded-md shadow m-4 text-white hover:shadow-lg hover:scale-110 transition-shadow duration-500 ">Volver
            al inicio</Link>
        </div>
      </div>
    </div>
