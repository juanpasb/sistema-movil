...
const FormularioSedes = (props: { sede?: Sede | undefined, event: any, modalProps: ModalProps }) => {
  const [btnDisabled, setBtnDisabled] = React.useState(false);

  const [nombre, setNombre] = React.useState("");
  const [direccion, setDireccion] = React.useState("");

  const eventForm = (sede: Sede | undefined): any => {
    return sede !== undefined
      ? props.event(sede.id, nombre, direccion, sede.estado)
      : props.event(nombre, direccion);
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    eventForm(props.sede).then((res: any) => {
      props.modalProps.exit(res.data.data, props.sede !== undefined);// Exit
      props.modalProps.message({// Message
  ...
  useEffect(() => {
    setNombre(props.sede?.nombre || "");
    setDireccion(props.sede?.direccion || "");
  }, []);

  return (
    <form onSubmit={(e) => handleSubmit(e)}>
      <div className="rounded-md p-8 grid grid-cols-1 lg:grid-cols-1 w-full gap-6">
      ...
      <div className="flex justify-end">
        <button
          type="submit"
          disabled={btnDisabled || nombre === props.sede?.nombre && direccion === props.sede?.direccion
            || nombre === "" || direccion === ""
          }
        ...
        >
          {btnDisabled && <Load />}
          {props.sede !== undefined ? "Actualizar" : "Guardar"}
        </button>
      </div>
    </form>
  );
...
export default FormularioSedes;